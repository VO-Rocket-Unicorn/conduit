# Remove `packages/conduit/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/conduit/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the conduit legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/conduit/src/storage/db.ts` are concentrated in 22 source files:

- `packages/conduit/src/account/repo.ts`
- `packages/conduit/src/cli/cmd/db.ts`
- `packages/conduit/src/cli/cmd/import.ts`
- `packages/conduit/src/cli/cmd/stats.ts`
- `packages/conduit/src/control-plane/workspace.ts`
- `packages/conduit/src/index.ts`
- `packages/conduit/src/node.ts`
- `packages/conduit/src/permission/index.ts`
- `packages/conduit/src/project/project.ts`
- `packages/conduit/src/server/projectors.ts`
- `packages/conduit/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/conduit/src/server/shared/fence.ts`
- `packages/conduit/src/session/message-v2.ts`
- `packages/conduit/src/session/projectors.ts`
- `packages/conduit/src/session/prompt.ts`
- `packages/conduit/src/session/session.ts`
- `packages/conduit/src/session/todo.ts`
- `packages/conduit/src/share/share-next.ts`
- `packages/conduit/src/storage/db.ts`
- `packages/conduit/src/sync/index.ts`
- `packages/conduit/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy conduit database wrapper; `packages/conduit/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the conduit wrapper.
...
Status: Completed. `SyncEvent` and the conduit projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy conduit database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy conduit database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy conduit database wrapper.
...
All migration groups are complete or superseded. `packages/conduit/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No conduit data-migration group remains.

The previous conduit `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate conduit data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/conduit`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/conduit/src`
- `bun typecheck` from `packages/conduit`
- Relevant package tests from `packages/conduit`, not the repo root
