...
Move the canonical Conduit terminal application from
`packages/conduit/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/conduit`,
`packages/cli`, or `@conduit-ai/core`.

The SDK is the TUI's Conduit boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/conduit`.
- Do not replace private `packages/conduit` imports with `@conduit-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@conduit-ai/tui`.
...
- Conduit domain data displayed by the TUI
...
```text
packages/conduit/src/cli/cmd/tui
```

Its private dependency on `packages/conduit` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/conduit/src/*`.
...
Status: Completed. The private `@conduit-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@conduit-ai/tui`.
...
- The package has no dependency on `conduit`, `@conduit-ai/cli`, or
  `@conduit-ai/core`.
...
primitives now live in `@conduit-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@conduit-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Conduit globals or process state;
...
Replace process-global Conduit state with resolved TUI inputs.
...
`@conduit-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/conduit` initially.
...
canonical in `@conduit-ai/tui`. Configured references resolve through the new
...
adapters rather than Conduit domain implementations.

Make the SDK the only Conduit domain boundary used by the TUI.
...
- Domain-facing TUI code imports Conduit data and operations only from
  `@conduit-ai/sdk`.
...
`@conduit-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/conduit`.
...
- The package has no imports from `packages/conduit`, `packages/cli`, or
  `@conduit-ai/core`.
...
public `@conduit-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/conduit`.
- Keep the legacy embedded worker and server startup in `packages/conduit`.
- Change those adapters to load config, create transport inputs, and call the
  public `@conduit-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/conduit`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@conduit-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@conduit-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Conduit domain state.
...
cd packages/conduit && bun typecheck
...
rg '@conduit-ai/core|packages/conduit|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/conduit/src packages/conduit/test
```
...
- Build the current-platform `packages/conduit` binary.
