...
The conduit server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@conduit-ai/llm` tool loop after replacing its
...
- plugins get global "conduit" instance like in that post i showed
- conduit instance has stuff like `conduit.session.prompt()` or
  `conduit.tool.register({...})`
...
- decide whether to preserve deprecated `@conduit-ai/llm` orchestration exports
