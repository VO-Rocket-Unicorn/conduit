...
Both `action` and `resource` use conduit's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.conduit` files is intentionally deferred until `.conduit` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether conduit permits an operation through its managed execution paths.
