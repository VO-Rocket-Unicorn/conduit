...
import { createConduit } from "@conduit-ai/sdk"
...
`)
  ...
  const conduit = await createConduit({ port: 0 })
  ...
  try {
    ...
    const session = await conduit.client.session.create()
    const result = await conduit.client.session
      .prompt({
