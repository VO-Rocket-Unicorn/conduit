...
Practical reference for a future `packages/server` split after the conduit
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/conduit`.
...
Until enough shared service code lives outside `packages/conduit`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/conduit`

It should not import `packages/conduit` services while `packages/conduit`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on conduit-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/conduit` implementation details.
