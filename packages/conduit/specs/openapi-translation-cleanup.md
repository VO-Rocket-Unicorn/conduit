...
Trim `packages/conduit/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/conduit/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/conduit`.
- `[x]` `bun typecheck` from `packages/conduit`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/conduit/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/conduit`.
- `[x]` `bun dev generate > /tmp/conduit-openapi.json` from `packages/conduit`.
...
- Edit `packages/conduit/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/conduit-openapi.json` from `packages/conduit`.
...
- Check source schemas in files like `packages/conduit/src/session/schema.ts`, `packages/conduit/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/conduit/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/conduit/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
