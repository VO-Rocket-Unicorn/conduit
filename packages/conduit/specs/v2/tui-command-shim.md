...
Delete `packages/conduit/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/conduit/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/conduit/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/conduit`
- TUI plugin loader tests in `packages/conduit` if runtime plugin API wiring changed
