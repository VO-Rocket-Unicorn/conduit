...
import { SessionV1 } from "@conduit-ai/core/v1/session"
import { FSUtil } from "@conduit-ai/core/fs-util"
import { ModelsDev } from "@conduit-ai/core/models-dev"
import { HttpRecorder } from "@conduit-ai/http-recorder"
import { HttpRecorderInternal } from "@conduit-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@conduit-ai/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@conduit-ai/llm/route"
import { Env } from "@/env"
...
const zenURL = (connection: string) => `https://console.conduit.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.CONDUIT_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["conduit", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("CONDUIT_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("CONDUIT_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["conduit", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "conduit-proxy",
    name: "Conduit proxy",
    providerID: ProviderV2.ID.conduit,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["conduit", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.CONDUIT_RECORD_CONSOLE_TOKEN && process.env.CONDUIT_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.conduit,
        name: "Conduit Zen",
        env: ["CONDUIT_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.CONDUIT_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.CONDUIT_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.CONDUIT_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["conduit", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("CONDUIT_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("CONDUIT_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("CONDUIT_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set CONDUIT_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the conduit LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "conduit.json"),
      JSON.stringify({ $schema: "https://conduit.ai/config.json", ...scenario.config(model) }),
    ),
  )
