...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "conduit.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "conduit.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "conduit.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "conduit.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["conduit.json", "conduit.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "conduit.json"),
        path.join(project, "conduit.jsonc"),
      ])
    })
  })
