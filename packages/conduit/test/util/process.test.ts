...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.CONDUIT_TEST ?? "")'), {
      env: {
        CONDUIT_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "CONDUIT_TEST_SHELL"], {
      shell: true,
      env: {
        CONDUIT_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("CONDUIT_TEST_SHELL=ok")
  })
