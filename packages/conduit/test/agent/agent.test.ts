...
it.instance("plan agent denies edits except .conduit/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".conduit/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".conduit", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.CONDUIT_TEST_HOME
      process.env.CONDUIT_TEST_HOME = test.directory
