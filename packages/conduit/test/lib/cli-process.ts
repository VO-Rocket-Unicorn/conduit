// Subprocess test harness for the conduit CLI. Spawns the real binary against
...
//
// Configuration flows through conduit's built-in test affordances:
//   - CONDUIT_CONFIG_CONTENT      : provider config inline, no files to find
//   - CONDUIT_TEST_HOME           : pins os.homedir() → tmpdir
//   - CONDUIT_DISABLE_PROJECT_CONFIG : skip walking up for conduit.json
//   - CONDUIT_PURE                : skip external plugin discovery + install
//   - CONDUIT_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `conduit.run` is fully wired. The shape supports adding more
// builders (`conduit.serve(opts)`, `conduit.acp(opts)`, `conduit.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of ConduitCli.
import { test, type TestOptions } from "bun:test"
import { FSUtil } from "@conduit-ai/core/fs-util"
import { AppProcess } from "@conduit-ai/core/process"
...
const conduitRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(conduitRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    CONDUIT_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    CONDUIT_CONFIG_CONTENT: configJson,
    CONDUIT_DISABLE_PROJECT_CONFIG: "1",
    CONDUIT_PURE: "1",
    CONDUIT_DISABLE_AUTOUPDATE: "1",
    CONDUIT_DISABLE_AUTOCOMPACT: "1",
    CONDUIT_DISABLE_MODELS_FETCH: "1",
    CONDUIT_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `conduit run`. New flags should
...
// `conduit serve` is a long-lived process — it never exits on its own.
...
// `conduit acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type ConduitCli = {
  // High-level: run a single prompt against the test model. Short-lived.
  readonly run: (message: string, opts?: RunOpts) => Effect.Effect<RunResult>
  // Spawn `conduit serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `conduit acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("conduit.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("conduit.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "conduit server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `conduit serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("conduit.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const conduit: ConduitCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, conduit })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `conduit.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "conduit") {
...
// every call site. Use this for any test that needs the conduit CLI fixture.
//
...
// resources (e.g. `conduit.serve`) without an extra `Effect.scoped` wrapper —
