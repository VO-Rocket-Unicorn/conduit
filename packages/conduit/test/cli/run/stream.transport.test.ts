...
import { ConduitClient, type GlobalEvent } from "@conduit-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<ConduitClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<ConduitClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<ConduitClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<ConduitClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<ConduitClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://conduit.test"),
...
function sdk(
  input: {
    ...
    subscribe?: ConduitClient["event"]["subscribe"]
    globalEvent?: ConduitClient["global"]["event"]
    promptAsync?: ConduitClient["session"]["promptAsync"]
    status?: ConduitClient["session"]["status"]
    messages?: ConduitClient["session"]["messages"]
    children?: ConduitClient["session"]["children"]
    permissions?: ConduitClient["permission"]["list"]
    questions?: ConduitClient["question"]["list"]
  } = {},
) {
  const client = new ConduitClient()

  const subscribe: ConduitClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: ConduitClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: ConduitClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: ConduitClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: ConduitClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: ConduitClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: ConduitClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: ConduitClient["question"]["list"] = input.questions ?? (() => ok([]))
