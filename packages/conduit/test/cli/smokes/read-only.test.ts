...
describe("conduit read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // CONDUIT_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ conduit }) =>
      Effect.gen(function* () {
        const r = yield* conduit.spawn(["mcp", "list"])
        conduit.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ conduit }) =>
      Effect.gen(function* () {
        const r = yield* conduit.spawn(["providers", "list"])
        conduit.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ conduit }) =>
      Effect.gen(function* () {
        const r = yield* conduit.spawn(["models"])
        conduit.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ conduit }) =>
      Effect.gen(function* () {
        const r = yield* conduit.spawn(["agent", "list"])
        conduit.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh CONDUIT_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ conduit }) =>
      Effect.gen(function* () {
        const r = yield* conduit.spawn(["session", "list"])
        conduit.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ conduit }) =>
      Effect.gen(function* () {
        const r = yield* conduit.spawn(["stats"])
        conduit.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under CONDUIT_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ conduit }) =>
      Effect.gen(function* () {
        const r = yield* conduit.spawn(["db", "path"])
        conduit.expectExit(r, 0, "db path")
