...
export function createAcpClient(input: Pick<CliFixture, "conduit">, env?: Record<string, string>) {
  return Effect.gen(function* () {
    return createJsonRpcAcpClient(yield* input.conduit.acp(env ? { env } : undefined))
  })
}
  ...
  return Effect.gen(function* () {
    return expectOk(
      yield* acp.request<InitializeResponse>("initialize", {
        ...
        clientInfo: { name: "conduit-local-acp", version: "0.1.0" },
      }),
    )
  })
}
