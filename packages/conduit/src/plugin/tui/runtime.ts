...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@conduit-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { errorData, errorMessage } from "@conduit-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@conduit-ai/tui/context/theme"
import { Global } from "@conduit-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@conduit-ai/core/util/flock"
import { Flag } from "@conduit-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@conduit-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@conduit-ai/core/v1/config/plugin"
import { createCommandShim } from "@conduit-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@conduit-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".conduit"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".conduit", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".conduit", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.CONDUIT_PURE ? [] : pluginOrigins
    if (Flag.CONDUIT_PURE && pluginOrigins.length) {
    }
