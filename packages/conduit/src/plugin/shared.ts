...
export const DEPRECATED_PLUGIN_PACKAGES = ["conduit-openai-codex-auth", "conduit-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, conduitVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(conduitVersion) || semver.major(conduitVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.conduit
  if (typeof range !== "string") return
  if (!semver.satisfies(conduitVersion, range)) {
    throw new Error(`Plugin requires conduit ${range} but running ${conduitVersion}`)
  }
}
