...
import type { LLMClientShape } from "@conduit-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("conduit"))
    return { type: "unsupported", reason: "provider is not openai, conduit, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @conduit-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains conduit-owned. The native runtime only adapts
      // the @conduit-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
