...
import type { Message, ConduitClient, SessionMessageResponse } from "@conduit-ai/sdk/v2"
...
export const AuthMethodID = "conduit-login"
...
export class Service extends Context.Service<Service, Interface>()("@conduit/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `conduit auth login` in the terminal",
      name: "Login with conduit",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: ConduitClient) {
...
function makeUsageService(sdk: ConduitClient) {
...
async function loadDirectorySnapshot(sdk: ConduitClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, conduit provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const conduitProvider = providers[ProviderV2.ID.make("conduit")]
  const conduitModel = conduitProvider ? Provider.sort(Object.values(conduitProvider.models))[0] : undefined
  if (conduitProvider && conduitModel) return { providerID: conduitProvider.id, modelID: conduitModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Conduit service failure", service })
}
