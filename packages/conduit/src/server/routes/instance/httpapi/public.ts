...
import { ConduitHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for conduit serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(ConduitHttpApi)
...
export const PublicApi = ConduitHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "conduit",
    version: "1.0.0",
