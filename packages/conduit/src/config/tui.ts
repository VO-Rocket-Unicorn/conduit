...
import { isRecord } from "@conduit-ai/tui/util/record"
...
import { TuiKeybind } from "@conduit-ai/tui/config/keybind"
...
import { TuiConfig } from "@conduit-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@conduit/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old conduit.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.conduit`
  // folders between cwd and home, and CONDUIT_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.CONDUIT_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit CONDUIT_TUI_CONFIG override, if set.
  if (Flag.CONDUIT_TUI_CONFIG) {
    const configFile = Flag.CONDUIT_TUI_CONFIG
  ...
  // 4. `.conduit` directories (and CONDUIT_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".conduit") || dir === Flag.CONDUIT_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".conduit") && dir !== Flag.CONDUIT_CONFIG_DIR) continue
