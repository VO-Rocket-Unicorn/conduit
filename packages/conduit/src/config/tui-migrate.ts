...
const TUI_SCHEMA_URL = "https://conduit.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from conduit.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const conduit = await conduitFiles(input)
  for (const file of conduit) {
...
async function conduitFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "conduit"),
    ...(await Filesystem.findUp(["conduit.json", "conduit.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "conduit"))
  }
  if (Flag.CONDUIT_CONFIG) files.push(Flag.CONDUIT_CONFIG)
