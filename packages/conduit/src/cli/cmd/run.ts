import type { PermissionV1 } from "@conduit-ai/core/v1/permission"
// CLI entry point for `conduit run`.
//
// Handles three modes:
...
//      conduit server and runs interactive mode against it.
//
...
import { createConduitClient, type ConduitClient, type ToolPart } from "@conduit-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<ConduitClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run conduit with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running conduit server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to CONDUIT_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to CONDUIT_SERVER_USERNAME or 'conduit')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: ConduitClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: ConduitClient, sessionID: string) {
      ...
      async function current(sdk: ConduitClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: ConduitClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: ConduitClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: ConduitClient) {
        ...
        async function loop(client: ConduitClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createConduitClient({
        baseUrl: "http://conduit.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
