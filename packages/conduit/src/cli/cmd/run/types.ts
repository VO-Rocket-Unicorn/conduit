...
import type { ConduitClient, PermissionRequest, QuestionRequest, ToolPart } from "@conduit-ai/sdk/v2"
import type { TuiConfig } from "@conduit-ai/tui/config"
...
type PromptModel = Parameters<ConduitClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<ConduitClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<ConduitClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<ConduitClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<ConduitClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<ConduitClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<ConduitClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<ConduitClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<ConduitClient["question"]["reject"]>[0]
