...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@conduit/acme")).toBe("@conduit/acme")
    expect(Npm.sanitize("@conduit/acme@1.0.0")).toBe("@conduit/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/conduit/acme.git"
    const expected = win ? "acme@git+https_//github.com/conduit/acme.git" : spec
