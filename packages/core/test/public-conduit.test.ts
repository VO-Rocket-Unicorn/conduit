...
import { AbsolutePath, Location, Model, Conduit, Session, Tool } from "@conduit-ai/core/public"
import { tmpdir } from "./fixture/tmpdir"
import { testEffect } from "./lib/effect"

const it = testEffect(Conduit.layer)

describe("public native Conduit API", () => {
  it.effect("exposes only the intentional Session capabilities", () =>
    Effect.gen(function* () {
      const conduit = yield* Conduit.Service

      expect(Object.keys(conduit).sort()).toEqual(["sessions", "tools"])

      expect(Object.keys(conduit.sessions).sort()).toEqual([
        "context",
        "create",
        "events",
        "get",
      ...
      expect(yield* conduit.sessions.list()).toBeArray()
      yield* conduit.tools.register({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          yield* conduit.sessions.create({
            id: sessionID,
            location: Location.Ref.make({ directory: AbsolutePath.make(tmp.path) }),
          })

          yield* conduit.sessions.switchModel({ sessionID, model })

          expect((yield* conduit.sessions.get(sessionID)).model).toEqual(model)
        }),
      ),
    ),
  )
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap(([available, disabled]) =>
        Effect.gen(function* () {
          ...
          yield* conduit.sessions.switchModel({ sessionID: availableID, model: ref({ variant: "default" }) })
          const disabledError = yield* conduit.sessions
            .switchModel({ sessionID: disabledID, model: ref() })
            .pipe(Effect.flip)
          const missingError = yield* conduit.sessions
          ...
          expect((yield* conduit.sessions.get(availableID)).model).toEqual(ref({ variant: "default" }))
          expect((yield* conduit.sessions.get(disabledID)).model).toBeUndefined()
        }),
      ),
    ),
  )
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          yield* conduit.sessions.switchModel({ sessionID, model: selected })

          const error = yield* conduit.sessions
            .switchModel({ sessionID, model: ref({ variant: "unknown" }) })
            .pipe(Effect.flip)

          expect(error).toBeInstanceOf(Session.VariantUnavailableError)
          expect((yield* conduit.sessions.get(sessionID)).model).toEqual(selected)
        }),
      ),
    ),
  )
