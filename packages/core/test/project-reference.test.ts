...
import { ConfigReference } from "@conduit-ai/core/config/reference"
...
import { ProjectReference } from "@conduit-ai/core/project-reference"
import { Repository } from "@conduit-ai/core/repository"
import { RepositoryCache } from "@conduit-ai/core/repository-cache"
...
describe("ProjectReference", () => {
  it.live("uses the broad experimental flag unless references are explicitly configured", () =>
    withEnv(
      { CONDUIT_EXPERIMENTAL: "true", CONDUIT_EXPERIMENTAL_REFERENCES: undefined },
      Effect.sync(() => {
        expect(Flag.CONDUIT_EXPERIMENTAL_REFERENCES).toBe(true)
      }),
    ).pipe(
      Effect.flatMap(() =>
        withEnv(
          { CONDUIT_EXPERIMENTAL: "true", CONDUIT_EXPERIMENTAL_REFERENCES: "false" },
          Effect.sync(() => {
            expect(Flag.CONDUIT_EXPERIMENTAL_REFERENCES).toBe(false)
          }),
        ),
      ),
    ),
...
function withReferences<A, E, R>(body: Effect.Effect<A, E, R>) {
  return withEnv({ CONDUIT_EXPERIMENTAL_REFERENCES: "true" }, body)
}

function withoutReferences<A, E, R>(body: Effect.Effect<A, E, R>) {
  return withEnv({ CONDUIT_EXPERIMENTAL: undefined, CONDUIT_EXPERIMENTAL_REFERENCES: undefined }, body)
}
