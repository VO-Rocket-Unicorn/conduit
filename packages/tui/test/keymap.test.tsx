...
import { getConduitModeStack, CONDUIT_BASE_MODE, ConduitKeymapProvider, registerConduitKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerConduitKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when conduit mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getConduitModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getConduitModeStack(keymap).push("autocomplete")
