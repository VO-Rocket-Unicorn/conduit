import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@conduit-ai/plugin/tui"
...
import type { useConduitKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useConduitKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getConduitModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getConduitModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
