...
export const CONDUIT_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const CONDUIT_MODE_KEY = "conduit.mode"

export const ConduitKeymapProvider = KeymapProvider
export const useConduitKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type ConduitModeStack = ReturnType<typeof createConduitModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, ConduitModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createConduitModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(CONDUIT_MODE_KEY, CONDUIT_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(CONDUIT_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(CONDUIT_MODE_KEY, stack.at(-1)?.mode ?? CONDUIT_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? CONDUIT_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(CONDUIT_MODE_KEY, undefined)
...
export function useConduitModeStack() {
  return getConduitModeStack(useConduitKeymap())
}

export function getConduitModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Conduit mode stack is not registered for this keymap")
  return value
}
...
export function registerConduitKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createConduitModeStack(keymap)
