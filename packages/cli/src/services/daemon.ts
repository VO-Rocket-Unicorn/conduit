...
import { createConduitClient } from "@conduit-ai/sdk/v2/client"
import { ServerAuth } from "@conduit-ai/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createConduitClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@conduit/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createConduitClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createConduitClient({ baseUrl: connection.url, headers: connection.headers })
    })
