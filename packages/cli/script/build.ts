...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      CONDUIT_CLI_NAME: `'${binary}'`,
      CONDUIT_MODELS_DEV: modelsData,
      CONDUIT_CHANNEL: `'${Script.channel}'`,
      CONDUIT_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
  ...
  await Bun.write(
    ...
    JSON.stringify(
      {
        name: `@conduit-ai/${name}`,
        version: Script.version,
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/anomalyco/conduit.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
