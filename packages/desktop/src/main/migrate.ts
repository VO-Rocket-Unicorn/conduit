...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.conduit.desktop.dev",
  beta: "ai.conduit.desktop.beta",
  prod: "ai.conduit.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.conduit.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `conduit.settings.dat` is special: it maps to the `conduit.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"conduit.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // conduit.settings.dat → the electron settings store ("conduit.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "conduit.global.dat").
  const storeName = filename === "conduit.settings.dat" ? "conduit.settings" : filename
