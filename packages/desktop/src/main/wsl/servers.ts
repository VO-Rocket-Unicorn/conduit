...
import { expectConduitVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveConduit?: typeof resolveWslConduit
...
export function createWslServersController(
  ...
  const setConduitCheck = (distro: string, check: WslConduitCheck) => {
    setState({
      conduitChecks: {
        ...state.conduitChecks,
        [distro]: check,
  ...
  const checkConduit = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveConduit ?? resolveWslConduit)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return conduitCheck(distro, resolved, version, appVersion)
  }

  const refreshConduitCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setConduitCheck(distro, await checkConduit(distro, opts))
  }
  ...
  const refreshConduitCheckBackground = (id: string, distro: string) => {
    void checkConduit(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setConduitCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl conduit check failed", { id, distro, message })
      })
  }

  const refreshConduitChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkConduit(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setConduitCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl conduit check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshConduitCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async probeConduit(name: string) {
      await runJob({ kind: "probe-conduit", distro: name, startedAt: Date.now() }, async (abort) => {
        await refreshConduitCheck(name, { signal: abort.signal })
      })
    },

    async installConduit(name: string) {
      await runJob({ kind: "install-conduit", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslConduit(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "Conduit installation failed")
        }
        await refreshConduitCheck(name, { signal: abort.signal })
        expectConduitVersion(state.conduitChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.conduitChecks, distro) : {}),
      })
    },
...
function conduitCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "conduit is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "conduit is installed but could not run",
    }
  }
  return {
    distro,
