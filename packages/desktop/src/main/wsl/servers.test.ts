...
let releaseConduitResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectConduitVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectConduitVersion("1.14.35", "1.16.2")).toThrow(
    "Conduit update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating Conduit", () => {
  expect(
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.conduit/bin/conduit",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, conduitChecks: {} })
})
...
test("ignores stale background Conduit checks after removing a WSL server", async () => {
  persistedServers = []
  releaseConduitResolve = undefined
  ...
  await waitFor(() => !!releaseConduitResolve)
  await controller.removeServer("wsl:Debian")
  releaseConduitResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().conduitChecks).toEqual({})
})

test("ignores stale startup Conduit checks after removing a WSL server", async () => {
...
function testControllerOptions() {
  return {
    ...
    resolveConduit: async () => {
      await new Promise<void>((resolve) => {
        releaseConduitResolve = resolve
      })
      return "/home/me/.conduit/bin/conduit"
    },
  }
}
