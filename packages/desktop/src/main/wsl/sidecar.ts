...
import { type WslCommandLine, resolveWslConduit, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const conduit = await resolveWslConduit(distro)
  if (!conduit) throw new Error(`Conduit is not installed in ${distro}`)
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export CONDUIT_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export CONDUIT_CLIENT=desktop",
    `export CONDUIT_SERVER_USERNAME=${shellEscape(username)}`,
    `export CONDUIT_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(conduit)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
