...
import type { UpdaterState } from "@conduit-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@conduit-ai/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.CONDUIT_CHANNEL === "prod" &&
...
const deepLinkEvent = "conduit:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__CONDUIT__ ??= {}
  const pending = window.__CONDUIT__.deepLinks ?? []
  window.__CONDUIT__.deepLinks = [...pending, ...urls]
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://conduit.ai/favicon-96x96-v3.png",
      })
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("conduit.global.dat").getItem("language")
