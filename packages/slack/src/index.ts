...
import { createConduit, type ToolPart } from "@conduit-ai/sdk"
...
console.log("🚀 Starting conduit server...")
const conduit = await createConduit({
  port: 0,
})
console.log("✅ Conduit server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await conduit.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new conduit session...")
    const { client, server } = conduit
    ...
    console.log("✅ Created conduit session:", createResult.data.id)
  ...
  console.log("📝 Sending to conduit:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Conduit response:", JSON.stringify(result, null, 2))
