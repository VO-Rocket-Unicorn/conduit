  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://conduit.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://conduit.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://conduit.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          ...
          lastProject: { local: "/local", "https://conduit.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://conduit.example.com"),
      ),
    ).toEqual({
