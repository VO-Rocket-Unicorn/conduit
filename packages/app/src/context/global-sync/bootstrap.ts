import type {
  ...
  Session,
  Todo,
} from "@conduit-ai/sdk/v2/client"
import { showToast } from "@/utils/toast"
import { getFilename } from "@conduit-ai/core/util/path"
import { retry } from "@conduit-ai/core/util/retry"
...
import { NormalizedProviderListResponse } from "@conduit-ai/ui/context"
...
export const loadGlobalConfigQuery = (scope: ServerScope, sdk: ConduitClient) =>
  queryOptions({
    queryKey: [scope, "config"],
    queryFn: () => retry(() => sdk.global.config.get().then((x) => x.data!)),
  })

export const loadProjectsQuery = (scope: ServerScope, sdk: ConduitClient) =>
  queryOptions({
    ...
    queryFn: () =>
      retry(() =>
        sdk.project.list().then((x) => {
          return (x.data ?? [])
            .filter((p) => !!p?.id)
            .filter((p) => !!p.worktree && !p.worktree.includes("conduit-test"))
            .slice()
...
export const loadProvidersQuery = (scope: ServerScope, directory: string | null, sdk: ConduitClient) =>
  queryOptions({
    queryKey: [scope, directory, "providers"],
    queryFn: () => retry(() => sdk.provider.list().then((x) => normalizeProviderList(x.data!))),
  })

export const loadAgentsQuery = (scope: ServerScope, directory: string | null, sdk: ConduitClient) =>
  queryOptions({
    queryKey: [scope, directory, "agents"],
    queryFn: () => retry(() => sdk.app.agents().then((x) => normalizeAgentList(x.data))),
  })

export const loadPathQuery = (scope: ServerScope, directory: string | null, sdk: ConduitClient) =>
  queryOptions<Path>({
