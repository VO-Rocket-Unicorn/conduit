...
import { Tag } from "@conduit-ai/ui/v2/badge-v2"
import { ButtonV2 } from "@conduit-ai/ui/v2/button-v2"
import { Dialog } from "@conduit-ai/ui/v2/dialog-v2"
import { Icon as IconV2 } from "@conduit-ai/ui/v2/icon"
import { IconButtonV2 } from "@conduit-ai/ui/v2/icon-button-v2"
import { MenuV2 } from "@conduit-ai/ui/v2/menu-v2"
...
import { wslConduitAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.conduitChecks[item.config.distro]
          const conduitAction = () => wslConduitAction(check())
          const busy = () => wsl.data?.job?.kind === "install-conduit" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={conduitAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installConduit(item.config.distro))}
                    >
