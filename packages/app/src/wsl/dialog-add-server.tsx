...
import { Spinner } from "@conduit-ai/ui/spinner"
import { showToast } from "@conduit-ai/ui/toast"
import { createEffect, createMemo, For, Match, onCleanup, Show, Switch } from "solid-js"
import { createStore } from "solid-js/store"
import { useLanguage } from "@/context/language"
import { usePlatform } from "@/context/platform"
import { useWslServers } from "./context"
import { enterWslConduitStep } from "./settings-model"

type WslServerStep = "wsl" | "distro" | "conduit"

const STEPS: WslServerStep[] = ["wsl", "distro", "conduit"]
...
export function DialogAddWslServer(props: DialogWslServerProps = {}) {
  ...
  const conduitCheck = createMemo(() => {
    const distro = selectedDistro()
    if (!distro) return null
    return current()?.conduitChecks[distro] ?? null
  })
  ...
  const conduitReady = createMemo(() => {
    const check = conduitCheck()
  ...
  const installingConduit = createMemo(() => {
    const job = current()?.job
    return job?.kind === "install-conduit" && job.distro === selectedDistro()
  })
  const allReady = createMemo(() => wslReady() && distroReady() && conduitReady())
  const addDisabled = createMemo(() => {
    const job = current()?.job
    if (!job) return store.adding
    return store.adding || job.kind !== "probe-conduit"
  })
  ...
  const autoProbe = createMemo(() => {
    ...
    if (!state.conduitChecks[distro]) {
      return { key: `probe-conduit:${distro}`, run: () => api.probeConduit(distro) }
    }
    return null
  })
  ...
  const conduitMessage = createMemo(() => {
    const state = current()
    if (!state) return language.t("wsl.onboarding.checkingConduit")
    const distro = selectedDistro()
    if (state.job?.kind === "install-conduit") {
      return distro
        ? language.t("wsl.onboarding.updatingConduitIn", { distro })
        : language.t("wsl.onboarding.updatingConduit")
    }
    if (state.job?.kind === "probe-conduit") {
      return distro
        ? language.t("wsl.onboarding.checkingConduitIn", { distro })
        : language.t("wsl.onboarding.checkingConduit")
    }
    if (conduitCheck()?.error) return conduitCheck()!.error
    if (conduitCheck()?.matchesDesktop === false) {
      return distro
        ? language.t("wsl.onboarding.updateConduitIn", { distro })
        : language.t("wsl.onboarding.updateConduit")
    }
    if (conduitReady()) {
      return distro
        ? language.t("wsl.onboarding.conduitReadyIn", { distro })
        : language.t("wsl.onboarding.conduitReady")
    }
    return distro
      ? language.t("wsl.onboarding.installConduitIn", { distro })
  ...
  const openConduitStep = () => {
    const distro = selectedDistro()
    if (!distro) return
    void run(() => enterWslConduitStep(distro, api.probeConduit, (step) => setStore("step", step)))
  }
        ...
        title:
          step === "wsl"
            ...
            : step === "distro"
              ...
              : language.t("wsl.onboarding.step.conduit"),
        state:
          active === step
            ? "current"
            : step === "wsl"
              ...
              : step === "distro"
                ? distroReady()
                  ...
                  : index > activeIndex
                    ? "locked"
                    : "warning"
                : conduitCheck()?.matchesDesktop === false
                  ? "warning"
                  : conduitReady()
                    ? "done"
                    : index > activeIndex
      ...
      >
        ...
        >
          ...
          <Switch>
            ...
            <Match when={activeStep() === "conduit"}>
              <div class="rounded-md bg-surface-base p-4 flex flex-col gap-3">
                <div class="flex items-center justify-between gap-3">
                  <div class="text-14-medium text-text-strong">{language.t("wsl.onboarding.step.conduit")}</div>
                  <div class="flex items-center gap-2">
                    <Show when={selectedDistro()}>
                      <Button
                        variant="ghost"
                        size="large"
                        disabled={busy()}
                        onClick={() => runSelectedDistro((distro) => api.probeConduit(distro))}
                      >
                        {language.t("wsl.onboarding.refresh")}
                      </Button>
                    </Show>
                    <Show when={!conduitReady() || conduitCheck()?.matchesDesktop === false}>
                      <Button
                        variant="secondary"
                        size="large"
                        disabled={busy()}
                        onClick={() => runSelectedDistro((distro) => api.installConduit(distro))}
                      >
                        <Show when={installingConduit()}>
                          <Spinner class="size-4 shrink-0" />
                        </Show>
                        {conduitCheck()?.resolvedPath
                          ? language.t("wsl.onboarding.updateConduit")
                          : language.t("wsl.onboarding.installConduit")}
                      </Button>
                    </Show>
                  </div>
                </div>
                <div class="text-12-regular text-text-weak whitespace-pre-wrap break-words">{conduitMessage()}</div>
                <Show when={conduitCheck()?.matchesDesktop === false ? conduitCheck() : null}>
                  {(check) => (
                    <div class="rounded-md border border-border-weak-base px-3 py-3 flex flex-col gap-1">
                      <div class="text-12-regular text-text-weak">
          ...
          <Show when={activeStep() === "conduit" && allReady() && selectedDistro()}>
