...
import { enterWslConduitStep, wslConduitAction, wslRuntimeRetryable } from "./settings-model"
...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when Conduit needs attention", () => {
    expect(wslConduitAction(undefined)).toBeUndefined()
    expect(
      wslConduitAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("Install Conduit")
    expect(
      wslConduitAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/conduit",
        version: "1.2.2",
        expectedVersion: "1.2.3",
  ...
  test("probes the selected distro before entering the Conduit step", async () => {
    const calls: string[] = []
    await enterWslConduitStep(
      "Debian",
      async (distro) => calls.push(distro),
      (step) => calls.push(step),
    )
    expect(calls).toEqual(["Debian", "conduit"])
  })
})
