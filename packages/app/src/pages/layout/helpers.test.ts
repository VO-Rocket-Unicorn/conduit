...
import { type Session } from "@conduit-ai/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("conduit://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("conduit://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("conduit://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("conduit://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("conduit://open-project")).toBeUndefined()
    expect(parseDeepLink("conduit://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "conduit://open-project?directory=/a",
      "conduit://other?directory=/b",
      "conduit://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("conduit://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("conduit://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("conduit://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("conduit://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "conduit://new-session?directory=/a",
      "conduit://open-project?directory=/b",
      "conduit://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __CONDUIT__: {
        deepLinks: ["conduit://open-project?directory=/a"],
      },
    } as unknown as Window & { __CONDUIT__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["conduit://open-project?directory=/a"])
