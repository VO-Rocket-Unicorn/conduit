...
const statsModelFallbackUrl = "https://stats.conduit.ai"
...
const worldCountries = feature<WorldCountryProperties>(worldTopology, worldCountryGeometries)
...
export default function StatsModel() {
  ...
  const modelDescription = createMemo(() =>
    stats()
      ? `${modelName()} usage, rank, token mix, cost, geo breakdown, and peer stats across Conduit.`
      : `${modelName()} model facts, limits, and Conduit usage availability.`,
  )
      ...
      <div data-slot="model-hero-grid">
        <div data-slot="model-hero-copy">
          ...
          <Show
            when={props.data}
            fallback={
              <p>Model facts from the shared model index. Conduit usage appears once this model has activity.</p>
            }
          >
            {(data) => (
              <p>
                Ranked #{data().rank} across recent Conduit token usage with {formatPercent(data().tokenShare)} of
                observed volume.
              </p>
            )}
          </Show>
...
function ModelCatalogCallout(props: { catalog: ModelCatalogEntry | null }) {
  return (
    <div data-component="model-rank-panel">
      ...
      <p>No Conduit usage in the current stats window.</p>
    </div>
  )
}
...
function ModelOverview(props: { data: StatsModelData | null }) {
  return (
    <section data-section="model-panel">
      ...
      <Show
        when={props.data}
        fallback={<ModelEmptyState title="No usage summary" description="This model has no Conduit usage rows yet." />}
      >
        {(data) => (
