...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve conduit route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "conduit")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "conduit")).toBe("openai")
    expect(statProvider("big-pickle", "", "conduit")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "conduit"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep conduit as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "conduit"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "conduit"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep conduit or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "conduit"), country: "US" })).toMatchObject([
