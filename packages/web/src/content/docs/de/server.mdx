---
title: Server
description: Nutze den conduit-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `conduit serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein conduit-Client nutzen kann.
...
Setze `CONDUIT_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `conduit`, kann aber mit `CONDUIT_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `conduit serve` und `conduit web`.
...
Wenn du `conduit` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den conduit-Server, um conduit programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `conduit serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `conduit serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Conduit-[IDE](/docs/ide)-Plugins.
...
Der conduit-Server stellt folgende APIs bereit.
