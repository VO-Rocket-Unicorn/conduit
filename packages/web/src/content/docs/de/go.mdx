...
Conduit Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in Conduit. Du abonnierst Conduit Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
Conduit zu verwenden.
...
Conduit Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

Conduit Go funktioniert wie jeder andere Provider in Conduit.

1. Du meldest dich bei **<a href={console}>Conduit Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Conduit Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Conduit Go abonnieren.
:::
...
Conduit Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Conduit Config verwendet das Format `conduit-go/<model-id>`. Für Kimi K2.6 würdest du beispielsweise `conduit-go/kimi-k2.6` in deiner Config verwenden.
...
Wir haben Conduit Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Conduit zu nutzen.
