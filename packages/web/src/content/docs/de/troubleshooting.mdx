...
Wenn Conduit Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\conduit\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `conduit --log-level DEBUG`.

---

## Speicher

conduit speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/conduit/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\conduit`
...
Conduit Desktop startet im Hintergrund einen lokalen Conduit-Server (`conduit-cli`-Sidecar).
...
- Nur macOS: `Conduit`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/conduit/conduit.jsonc` (oder `~/.config/conduit/conduit.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/conduit/conduit.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\conduit\conduit.jsonc`
...
Conduit kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/conduit/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\conduit\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Conduit ihn neu aufbauen kann.

1. Beende Conduit Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/conduit`
- **Linux**: loesche `~/.cache/conduit` (oder fuehre aus: `rm -rf ~/.cache/conduit`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\conduit`

3. Starte Conduit Desktop neu.

---

### Server-Verbindungsprobleme beheben

Conduit Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `conduit.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `CONDUIT_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `CONDUIT_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Conduit Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Conduit-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Conduit Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Conduit in den OS-Einstellungen aktiviert sind, und
...
1. Beende Conduit Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Conduit Desktop):

- `conduit.settings.dat` (desktop default server URL)
- `conduit.global.dat` und `conduit.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Conduit hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `conduit models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\conduit`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. conduit installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\conduit`

2. Starte conduit neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt conduit, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
conduit erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
