...
Agent Skills erlauben Conduit, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Conduit sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Conduit vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.conduit/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/conduit/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.conduit/skills/git-release/SKILL.md` zum Beispiel so:
...
Conduit listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `conduit.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `conduit.json`):
