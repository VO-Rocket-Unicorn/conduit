---
title: Regeln
description: Lege eigene Anweisungen fuer conduit fest.
---

Du kannst conduit ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in conduit den Befehl `/init` aus.
...
So kann conduit sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht conduit sie zu erweitern.
...
conduit kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/conduit/AGENTS.md` speichern.
Diese gelten in allen conduit-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Conduit dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/conduit/AGENTS.md` existiert)
...
Beim Start sucht conduit in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/conduit/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `conduit.json` oder in `~/.config/conduit/conduit.json` angeben.
...
conduit parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit conduit.json

Empfohlen ist das `instructions`-Feld in der `conduit.json`:
...
Du kannst conduit explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `conduit.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
