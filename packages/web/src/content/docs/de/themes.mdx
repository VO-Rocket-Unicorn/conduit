...
Mit Conduit kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Conduit das `conduit`-Theme.
...
Conduit liefert mehrere eingebaute Themes mit.
...
- Conduit optisch ans Terminal anpassen wollen
...
Conduit unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/conduit/themes/*.json` oder `$XDG_CONFIG_HOME/conduit/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.conduit/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.conduit/themes/*.json`
