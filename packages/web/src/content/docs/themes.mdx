...
With Conduit you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Conduit uses our own `conduit` theme.
...
Conduit comes with several built-in themes.
...
- Want Conduit to match their terminal's appearance
...
Conduit supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/conduit/themes/*.json` or `$XDG_CONFIG_HOME/conduit/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.conduit/themes/*.json`
4. **Current working directory** - Defined in `./.conduit/themes/*.json`
