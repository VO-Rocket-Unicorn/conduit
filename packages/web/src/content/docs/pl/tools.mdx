...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Conduit posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Conduit.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `CONDUIT_EXPERIMENTAL_LSP_TOOL=true` (lub `CONDUIT_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Conduit lub gdy zmienna środowiskowa `CONDUIT_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Conduit:
