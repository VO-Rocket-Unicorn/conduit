...
Umiejętności agenta pozwalają conduit odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
conduit przeszukuje te lokalizacje:

- Project config: `.conduit/skills/<name>/SKILL.md`
- Global config: `~/.config/conduit/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu conduit przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.conduit/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/conduit/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.conduit/skills/git-release/SKILL.md` w ten sposób:
...
conduit wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `conduit.json`:
...
**Dla agentów wbudowanych** (w `conduit.json`):
