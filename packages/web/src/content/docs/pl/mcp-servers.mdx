...
Możesz dodać dodatkowe informacje do conduit za pomocą _Model Context Protocol_, w skrócie MCP. conduit obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [conduit Config](https://conduit.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/conduit`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
conduit automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, conduit:
...
Jeśli serwer wymaga uwierzytelnienia, conduit poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `conduit mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji conduit bezpieczne przechowa tokeny w `~/.local/share/conduit/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w conduit, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie conduit, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć conduit z kontem Sentry.
