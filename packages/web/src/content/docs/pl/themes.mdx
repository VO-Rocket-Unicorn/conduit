...
Dzięki conduit możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie conduit używa naszego własnego motywu `conduit`.
...
conduit ma kilka wbudowanych motywów.
...
- Chcą, aby Conduit pasował do wyglądu ich terminala
...
conduit obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/conduit/themes/*.json` lub `$XDG_CONFIG_HOME/conduit/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.conduit/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.conduit/themes/*.json`
