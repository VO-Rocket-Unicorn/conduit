---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w conduit.
---

import config from "../../../../config.mjs"
export const console = config.console

conduit używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji conduit.
...
w `~/.local/share/conduit/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim conduit
config.
...
Conduit Zen to lista modeli dostarczonych przez zespół conduit, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z conduit. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Conduit Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Conduit Zen` i przejdź do [conduit.ai/auth](https://conduit.ai/zen).
...
Działa jak każdy inny dostawca w conduit i jest całkowicie opcjonalny w użyciu.

---

## Conduit Go

Conduit Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół conduit, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z conduit.

1. Uruchom polecenie `/connect` w TUI, wybierz `Conduit Go` i przejdź do [conduit.ai/auth](https://conduit.ai/zen).
...
Aby używać Amazon Bedrock z conduit:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania conduit:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `conduit.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w conduit nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować conduit do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod conduit działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji conduit.
...
conduit używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować conduit
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`conduit.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [conduit-gitlab-auth](https://www.npmjs.com/package/conduit-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem conduit:
...
Aby używać Google Vertex AI z conduit:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania conduit.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą conduit, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [conduit-helicone-session](https://github.com/H2Shami/conduit-helicone-session), aby automatycznie rejestrować każdą konwersację conduit jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja conduit będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować conduit tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować conduit tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować conduit tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem conduit. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/conduit).
:::
...
Aby korzystać z Ollama Cloud z conduit:
...
3. Skopiuj klucz API do użycia w conduit.
...
6. **Ważne**: Przed użyciem modeli chmurowych w conduit musisz pobrać informacje o modelu lokalnie:
...
Conduit Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół conduit. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Conduit Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Conduit Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API conduit.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji conduit.
...
5. Możesz je także dostosować za pomocą konfiguracji conduit. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z conduit:
...
Możesz także dostosować modele za pomocą konfiguracji conduit. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z conduit. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `conduit.json` w katalogu projektu:
...
Pola `limit` pozwalają conduit zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `conduit auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację conduit i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji conduit.
