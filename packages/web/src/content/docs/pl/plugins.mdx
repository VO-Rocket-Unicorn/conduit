---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć conduit.
---

Wtyczki umożliwiające wyłączenie conduit poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania conduit.
...
- `.conduit/plugins/` - Wtyczki na poziomie projektu
- `~/.config/conduit/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/conduit/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/conduit/conduit.json`)
2. Konfiguracja projektu (`conduit.json`)
3. Globalny katalog wtyczek (`~/.config/conduit/plugins/`)
4. Katalog wtyczek projektu (`.conduit/plugins/`)
...
conduit uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z conduit do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do conduit.
...
:::note
Jeśli korzystasz z aplikacji komputerowej conduit, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj conduit czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do conduit:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod conduit. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://conduit.ai/docs/sdk).
