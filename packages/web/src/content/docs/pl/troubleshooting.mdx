...
Aby debugować problemy z conduit, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\conduit\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `conduit --log-level DEBUG`.

---

## Przechowywanie danych

conduit przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/conduit/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\conduit`
...
conduit Desktop uruchamia w tle lokalny serwer conduit (sidecar `conduit-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Conduit` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/conduit/conduit.jsonc` (lub `~/.config/conduit/conduit.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/conduit/conduit.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\conduit\conduit.jsonc`
...
conduit może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/conduit/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\conduit\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby conduit mógł ją odbudować.

1. Całkowicie zamknij conduit Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/conduit`
- **Linux**: usuń `~/.cache/conduit` (lub uruchom `rm -rf ~/.cache/conduit`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\conduit`

3. Uruchom ponownie conduit Desktop.

---

### Napraw problemy z połączeniem z serwerem

conduit Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `conduit.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `CONDUIT_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `CONDUIT_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows conduit Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami conduit.

---

### Brak powiadomień

conduit Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla conduit w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie conduit Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji conduit Desktop):

- `conduit.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `conduit.global.dat` i `conduit.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `conduit upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `conduit models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\conduit`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. conduit dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\conduit`

2. Uruchom ponownie kod conduit, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to conduit do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
conduit wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
