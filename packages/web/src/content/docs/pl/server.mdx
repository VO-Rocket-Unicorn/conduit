---
title: Serwer
description: Interakcja z serwerem conduit za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `conduit serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient conduit.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `conduit.local` |
...
Ustaw `CONDUIT_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `conduit` lub ustaw `CONDUIT_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `conduit serve`, jak i `conduit web`.
...
Kiedy uruchomisz `conduit`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera conduit do programowej interakcji z conduit.
:::

Ta architektura umożliwia obsługę wielu klientów przez conduit i programową interakcję z kodem otwartym.

Możesz uruchomić `conduit serve`, aby uruchomić samodzielny serwer. Jeśli masz
conduit TUI działa, `conduit serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki conduit [IDE](/docs/ide).
...
Serwer conduit udostępnia następujące interfejsy API.
