...
Create `.conduit/commands/test.md`:
...
You can add custom commands through the Conduit config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Conduit [config](/docs/config):
...
conduit includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
