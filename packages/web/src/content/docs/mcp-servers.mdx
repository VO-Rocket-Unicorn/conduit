...
You can add external tools to Conduit using the _Model Context Protocol_, or MCP. Conduit supports both local and remote servers.
...
You can define MCP servers in your [Conduit Config](https://conduit.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/conduit` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Conduit automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Conduit will:
...
If the server requires authentication, Conduit will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `conduit mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Conduit will store the tokens securely in `~/.local/share/conduit/mcp-auth.json`.
...
Your MCPs are available as tools in Conduit, alongside built-in tools. So you can manage them through the Conduit config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Conduit to your Sentry account.
