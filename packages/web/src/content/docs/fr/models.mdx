...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez Conduit.
...
Voici plusieurs modèles qui fonctionnent bien avec Conduit, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration Conduit.

```json title="conduit.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [Conduit Zen](/docs/zen), vous utiliserez `conduit/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="conduit.jsonc" {7-12,19-24}
{
  "$schema": "https://conduit.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="conduit.jsonc" {6-21}
{
  "$schema": "https://conduit.ai/config.json",
  "provider": {
    "conduit": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. Conduit est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

Conduit est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="conduit.jsonc" {7-18}
{
  "$schema": "https://conduit.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque Conduit démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration Conduit.
