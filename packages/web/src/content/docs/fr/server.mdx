---
title: Serveur
description: Interagissez avec le serveur conduit via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `conduit serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client conduit peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `conduit.local` |
...
Définissez `CONDUIT_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `conduit`, ou définissez `CONDUIT_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `conduit serve` et à `conduit web`.
...
Lorsque vous exécutez `conduit`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur conduit pour interagir avec conduit par programmation.
:::

Cette architecture permet à conduit de prendre en charge plusieurs clients et vous permet d'interagir avec conduit par programme.

Vous pouvez exécuter `conduit serve` pour démarrer un serveur autonome. Si vous avez le
conduit TUI en cours d'exécution, `conduit serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Conduit [IDE](/docs/ide).
...
Le serveur conduit expose les API suivantes.
