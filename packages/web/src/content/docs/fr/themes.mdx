...
Avec Conduit, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Conduit utilise notre propre thème `conduit`.
...
Conduit est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Conduit corresponde à l'apparence de leur terminal
...
Conduit prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/conduit/themes/*.json` ou `$XDG_CONFIG_HOME/conduit/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.conduit/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.conduit/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/conduit/themes
vim ~/.config/conduit/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .conduit/themes
vim .conduit/themes/my-theme.json
```
...
{
  "$schema": "https://conduit.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
