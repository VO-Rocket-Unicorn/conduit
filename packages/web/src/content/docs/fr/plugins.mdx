---
title: Plugins
description: Écrivez vos propres plugins pour étendre Conduit.
---

Les plugins vous permettent d'étendre Conduit en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Conduit.
...
- `.conduit/plugins/` - Plugins au niveau du projet
- `~/.config/conduit/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/conduit/node_modules/`.
...
1. Configuration globale (`~/.config/conduit/conduit.json`)
2. Configuration du projet (`conduit.json`)
3. Répertoire global des plugins (`~/.config/conduit/plugins/`)
4. Répertoire des plugins du projet (`.conduit/plugins/`)
...
Conduit exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK conduit pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre conduit.
...
:::note
Si vous utilisez l'application de bureau Conduit, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez conduit de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à conduit :
...
L'assistant `tool` crée un outil personnalisé que conduit peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour conduit aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://conduit.ai/docs/sdk) pour plus de détails.
