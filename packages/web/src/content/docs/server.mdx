---
title: Server
description: Interact with conduit server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `conduit serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an conduit client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `conduit.local` |
...
Set `CONDUIT_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `conduit`, or set `CONDUIT_SERVER_USERNAME` to override it. This applies to both `conduit serve` and `conduit web`.
...
When you run `conduit` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the conduit server to interact with conduit programmatically.
:::

This architecture lets conduit support multiple clients and allows you to interact with conduit programmatically.

You can run `conduit serve` to start a standalone server. If you have the
conduit TUI running, `conduit serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Conduit [IDE](/docs/ide) plugins.
...
The conduit server exposes the following APIs.
