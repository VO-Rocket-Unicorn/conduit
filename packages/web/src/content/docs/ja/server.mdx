---
title: サーバー
description: HTTP 経由で Conduit サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`conduit serve` コマンドは、Conduit クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `conduit.local` |
...
HTTP 基本認証でサーバーを保護するには、`CONDUIT_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `conduit` になるか、`CONDUIT_SERVER_USERNAME` を設定してオーバーライドします。これは、`conduit serve` と `conduit web` の両方に当てはまります。
...
`conduit` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Conduit サーバーを使用して、プログラムで Conduit と対話します。
:::

`conduit serve` を実行してスタンドアロンサーバーを起動できます。すでに Conduit TUI を実行している場合でも、`conduit serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Conduit [IDE](/docs/ide) プラグインによって使用されます。
