...
Con Conduit puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Conduit usa nuestro propio tema `conduit`.
...
Conduit viene con varios temas integrados.
...
- Quiere que Conduit coincida con la apariencia de su terminal
...
Conduit admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/conduit/themes/*.json` o `$XDG_CONFIG_HOME/conduit/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.conduit/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.conduit/themes/*.json`
