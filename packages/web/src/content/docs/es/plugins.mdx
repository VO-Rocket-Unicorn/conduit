---
title: Complementos
description: Escriba sus propios complementos para extender Conduit.
---

Los complementos le permiten extender Conduit al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Conduit.
...
- `.conduit/plugins/` - Complementos a nivel de proyecto
- `~/.config/conduit/plugins/` - Complementos globales
...
```json title="conduit.json"
{
  ...
  "plugin": ["conduit-helicone-session", "conduit-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/conduit/node_modules/`.
...
1. Configuración global (`~/.config/conduit/conduit.json`)
2. Configuración del proyecto (`conduit.json`)
3. Directorio global de complementos (`~/.config/conduit/plugins/`)
4. Directorio de complementos del proyecto (`.conduit/plugins/`)
...
```json title=".conduit/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Conduit ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".conduit/plugins/my-plugin.ts"
...
```js title=".conduit/plugins/example.js"
...
- `client`: Un cliente SDK conduit para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar conduit.
...
```js title=".conduit/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "conduit"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Conduit, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que conduit lea archivos `.env`:

```javascript title=".conduit/plugins/env-protection.js"
...
```javascript title=".conduit/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a conduit:

```ts title=".conduit/plugins/custom-tools.ts"
import { type Plugin, tool } from "@conduit-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que conduit puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para conduit junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://conduit.ai/docs/sdk) para obtener más detalles.
...
```ts title=".conduit/plugins/compaction.ts"
import type { Plugin } from "@conduit-ai/plugin"
...
```ts title=".conduit/plugins/custom-compaction.ts"
import type { Plugin } from "@conduit-ai/plugin"
