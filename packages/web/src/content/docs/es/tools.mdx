...
Las herramientas permiten que LLM realice acciones en su código base. Conduit viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Conduit.
...
:::note
Esta herramienta solo está disponible cuando `CONDUIT_EXPERIMENTAL_LSP_TOOL=true` (o `CONDUIT_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Conduit o cuando la variable de entorno `CONDUIT_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Conduit:

```bash
CONDUIT_ENABLE_EXA=1 conduit
```
