...
Las habilidades del agente permiten a Conduit descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Conduit busca estas ubicaciones:

- Configuración del proyecto: `.conduit/skills/<name>/SKILL.md`
- Configuración global: `~/.config/conduit/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Conduit sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.conduit/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/conduit/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.conduit/skills/git-release/SKILL.md` así:
...
Conduit enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `conduit.json`:
...
**Para agentes integrados** (en `conduit.json`):
