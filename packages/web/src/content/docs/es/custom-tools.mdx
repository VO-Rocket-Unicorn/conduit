...
description: Cree herramientas que LLM pueda llamar en conduit.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de conduit como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.conduit/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/conduit/tools/`.
...
```ts title=".conduit/tools/database.ts" {1}
...
```ts title=".conduit/tools/math.ts"
import { tool } from "@conduit-ai/plugin"
...
```ts title=".conduit/tools/bash.ts"
import { tool } from "@conduit-ai/plugin"
...
```ts title=".conduit/tools/project.ts" {8}
import { tool } from "@conduit-ai/plugin"
...
```python title=".conduit/tools/add.py"
import sys
...
```ts title=".conduit/tools/python-add.ts" {10}
import { tool } from "@conduit-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".conduit/tools/add.py")
