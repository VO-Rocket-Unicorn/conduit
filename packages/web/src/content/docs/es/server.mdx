---
title: Servidor
description: Interactuar con el servidor conduit a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `conduit serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente conduit puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `conduit.local` |
...
Configure `CONDUIT_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `conduit`, o configure `CONDUIT_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `conduit serve` como a `conduit web`.
...
Cuando ejecuta `conduit`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor conduit para interactuar con conduit mediante programación.
:::

Esta arquitectura permite que conduit admita múltiples clientes y le permite interactuar con conduit mediante programación.

Puede ejecutar `conduit serve` para iniciar un servidor independiente. Si tienes el
conduit TUI ejecutándose, `conduit serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Conduit [IDE](/docs/ide).
...
El servidor conduit expone las siguientes API.
