...
As habilidades do agente permitem que o conduit descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O conduit pesquisa nesses locais:

- Configuração do projeto: `.conduit/skills/<name>/SKILL.md`
- Configuração global: `~/.config/conduit/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o conduit sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.conduit/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/conduit/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.conduit/skills/git-release/SKILL.md` assim:
...
O conduit lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `conduit.json`:
...
**Para agentes embutidos** (em `conduit.json`):
