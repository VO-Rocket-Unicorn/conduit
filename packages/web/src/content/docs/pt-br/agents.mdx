...
Existem dois tipos de agentes no conduit; agentes primários e subagentes.
...
conduit vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Conduit vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Conduit vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Conduit, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `conduit.json`:
...
- Global: `~/.config/conduit/agents/`
- Por projeto: `.conduit/agents/`

```markdown title="~/.config/conduit/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o conduit usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do conduit quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do conduit usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Conduit Zen](/docs/zen), você usaria `conduit/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="conduit.json" {3-6,9-12}
{
  "$schema": "https://conduit.ai/config.json",
  "tools": {
    "write": true,
...
```json title="conduit.json" {3-5,8-10}
{
  "$schema": "https://conduit.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `conduit models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/conduit).
:::
...
```markdown title="~/.config/conduit/agents/docs-writer.md"
---
...
```markdown title="~/.config/conduit/agents/security-auditor.md"
---
