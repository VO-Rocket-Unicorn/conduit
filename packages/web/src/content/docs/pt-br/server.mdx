---
title: Servidor
description: Interaja com o servidor conduit via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `conduit serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente conduit pode usar.
...
```bash
conduit serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `conduit.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
conduit serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `CONDUIT_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `conduit`, ou defina `CONDUIT_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `conduit serve` quanto ao `conduit web`.

```bash
CONDUIT_SERVER_PASSWORD=your-password conduit serve
```
...
Quando você executa `conduit`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor conduit para interagir com o conduit programaticamente.
:::

Essa arquitetura permite que o conduit suporte múltiplos clientes e permite que você interaja com o conduit programaticamente.

Você pode executar `conduit serve` para iniciar um servidor autônomo. Se você tiver o TUI do conduit em execução, `conduit serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do conduit [IDE](/docs/ide).
...
O servidor conduit expõe as seguintes APIs.
