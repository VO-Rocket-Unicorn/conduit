...
Com o conduit, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o conduit usa nosso próprio tema `conduit`.
...
O conduit vem com vários temas embutidos.
...
- Querem que o conduit corresponda à aparência do seu terminal
...
O conduit suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/conduit/themes/*.json` ou `$XDG_CONFIG_HOME/conduit/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.conduit/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.conduit/themes/*.json`
