---
title: Plugins
description: Escreva seus próprios plugins para estender o conduit.
---

Plugins permitem que você estenda o conduit conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do conduit.
...
- `.conduit/plugins/` - Plugins em nível de projeto
- `~/.config/conduit/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/conduit/node_modules/`.
...
1. Configuração global (`~/.config/conduit/conduit.json`)
2. Configuração do projeto (`conduit.json`)
3. Diretório de plugins global (`~/.config/conduit/plugins/`)
4. Diretório de plugins do projeto (`.conduit/plugins/`)
...
O conduit executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do conduit para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o conduit.
...
:::note
Se você estiver usando o aplicativo desktop conduit, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o conduit de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao conduit:
...
O helper `tool` cria uma ferramenta personalizada que o conduit pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o conduit junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://conduit.ai/docs/sdk) para detalhes.
