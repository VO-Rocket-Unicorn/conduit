...
يمكنك إضافة أدوات خارجية إلى Conduit باستخدام _Model Context Protocol_، أو MCP. يدعم Conduit خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Conduit](https://conduit.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/conduit` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Conduit تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Conduit بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Conduit المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `conduit mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Conduit الرموز بشكل آمن في `~/.local/share/conduit/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Conduit إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Conduit مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Conduit بحساب Sentry الخاص بك.
