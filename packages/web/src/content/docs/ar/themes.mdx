...
مع Conduit يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Conduit سمتنا `conduit`.
...
يأتي Conduit مع عدة سمات مدمجة.
...
- يريدون أن يطابق Conduit مظهر terminal لديهم
...
يدعم Conduit نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/conduit/themes/*.json` أو `$XDG_CONFIG_HOME/conduit/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.conduit/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.conduit/themes/*.json`
