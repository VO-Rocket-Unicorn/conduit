---
title: الخادم
description: تفاعل مع خادم conduit عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `conduit serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل conduit استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `conduit.local` |
...
عيّن `CONDUIT_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `conduit`، أو عيّن `CONDUIT_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `conduit serve` و `conduit web`.
...
عند تشغيل `conduit` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم conduit للتفاعل مع conduit برمجيا.
:::

تتيح هذه البنية لـ conduit دعم عدة عملاء وتمكّنك من التفاعل مع conduit برمجيا.

يمكنك تشغيل `conduit serve` لبدء خادم مستقل. إذا كانت واجهة conduit في terminal (TUI)
قيد التشغيل، فسيبدأ `conduit serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Conduit لـ [IDE](/docs/ide).
...
يعرض خادم conduit واجهات API التالية.
