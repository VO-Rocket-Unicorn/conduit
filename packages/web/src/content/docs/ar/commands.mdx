...
أنشئ الملف `.conduit/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Conduit أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Conduit:
...
- على مستوى النظام: `~/.config/conduit/commands/`
- لكل مشروع: `.conduit/commands/`
...
يتضمن conduit عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
