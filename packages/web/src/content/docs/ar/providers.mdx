---
title: المزوّدون
description: استخدام أي موفّر LLM في Conduit.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Conduit ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Conduit.
...
في `~/.local/share/conduit/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Conduit.
...
Conduit Zen هي قائمة نماذج يوفّرها فريق Conduit وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Conduit. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Conduit Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر conduit، ثم انتقل إلى [conduit.ai/auth](https://conduit.ai/auth).
...
يعمل مثل أي مزوّد آخر في Conduit واستخدامه اختياري بالكامل.

---

## Conduit Go

Conduit Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Conduit، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Conduit.

1. شغّل الأمر `/connect` في TUI، واختر `Conduit Go`، ثم انتقل إلى [conduit.ai/auth](https://conduit.ai/zen).
...
لاستخدام Amazon Bedrock مع Conduit:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل conduit:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `conduit.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Conduit غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين conduit لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل conduit بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات conduit.
...
تخزين الرمز في مساحة تخزين مصادقة conduit.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Conduit نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Conduit
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`conduit.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [conduit-gitlab-auth](https://www.npmjs.com/package/conduit-gitlab-auth).

##### التهيئة

خصّص عبر `conduit.json`:
...
لاستخدام اشتراك GitHub Copilot مع conduit:
...
لاستخدام Google Vertex AI مع Conduit:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر conduit، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [conduit-helicone-session](https://github.com/H2Shami/conduit-helicone-session) لتسجيل كل محادثة Conduit تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Conduit مدرجة كجلسة مستقلة.
...
يمكنك ضبط conduit لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط conduit لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط conduit لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Conduit. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/conduit) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Conduit:
...
3. انسخ مفتاح API لاستخدامه في Conduit.
...
6. **مهم**: قبل استخدام نماذج السحابة في Conduit، يجب سحب معلومات النموذج محليا:
...
Conduit Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Conduit. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Conduit Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Conduit Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Conduit.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات conduit.
...
5. يمكنك أيضا تخصيصها عبر إعدادات conduit. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Conduit:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات conduit. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع conduit. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `conduit.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Conduit بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `conduit auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات conduit وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات conduit.
