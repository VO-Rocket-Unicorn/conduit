---
title: Plugin
description: Scrivi plugin per estendere Conduit.
---

I plugin ti permettono di estendere Conduit agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Conduit.
...
- `.conduit/plugins/` - plugin a livello progetto
- `~/.config/conduit/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/conduit/node_modules/`.
...
1. Config globale (`~/.config/conduit/conduit.json`)
2. Config di progetto (`conduit.json`)
3. Directory plugin globale (`~/.config/conduit/plugins/`)
4. Directory plugin di progetto (`.conduit/plugins/`)
...
Conduit esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di conduit per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere conduit.
...
:::note
Se usi l'app desktop di Conduit, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a conduit di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a conduit:
...
L'helper `tool` crea uno strumento personalizzato che conduit può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in conduit insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://conduit.ai/docs/sdk) per i dettagli.
