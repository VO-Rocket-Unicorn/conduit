---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Conduit.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Conduit fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Conduit avvia la TUI per la directory corrente.
...
Quando usi la TUI di Conduit, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Conduit. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da Conduit. _Alias_: `/quit`, `/q`
...
Questo è separato da `conduit.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo Conduit attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `conduit.default`.
