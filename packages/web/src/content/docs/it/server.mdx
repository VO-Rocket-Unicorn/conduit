---
title: Server
description: Interagisci con il server conduit via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `conduit serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client conduit.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `conduit.local` |
...
Imposta `CONDUIT_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `conduit`, oppure imposta `CONDUIT_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `conduit serve` sia per `conduit web`.
...
Quando esegui `conduit` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server conduit per interagire con conduit in modo programmatico.
:::

Questa architettura permette a conduit di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `conduit serve` per avviare un server standalone. Se la TUI di conduit e gia in esecuzione, `conduit serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Conduit.
...
Il server conduit espone le seguenti API.
