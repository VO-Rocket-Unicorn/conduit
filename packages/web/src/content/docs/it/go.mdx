...
Conduit Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in Conduit. Ti abboni a Conduit Go e
...
Conduit Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

Conduit Go funziona come qualsiasi altro provider in Conduit.

1. Accedi a **<a href={console}>Conduit Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `Conduit Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a Conduit Go.
:::
...
Conduit Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua Conduit config
utilizza il formato `conduit-go/<model-id>`. Ad esempio, per Kimi K2.6, useresti
`conduit-go/kimi-k2.6` nella tua configurazione.
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Conduit.
