...
Le skill degli agenti permettono a Conduit di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Conduit cerca in queste posizioni:

- Config di progetto: `.conduit/skills/<name>/SKILL.md`
- Config globale: `~/.config/conduit/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Conduit risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.conduit/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/conduit/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.conduit/skills/git-release/SKILL.md` cosi':
...
Conduit elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `conduit.json`:
...
**Per agenti integrati** (in `conduit.json`):
