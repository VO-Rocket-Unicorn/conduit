...
description: Usa la configurazione JSON di Conduit.
---

Puoi configurare Conduit usando un file di configurazione JSON.

---

## Formato

Conduit supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
1. **Config remota** (da `.well-known/conduit`) - default dell'organizzazione
2. **Config globale** (`~/.config/conduit/conduit.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `CONDUIT_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`conduit.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.conduit`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `CONDUIT_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.conduit` e `~/.config/conduit` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/conduit`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di Conduit in `~/.config/conduit/conduit.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/conduit/tui.json`.
...
Aggiungi `conduit.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando Conduit si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `CONDUIT_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `CONDUIT_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.conduit` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.conduit`, quindi **puo sovrascrivere** le loro impostazioni.

---

## Schema

Il file di configurazione ha uno schema definito in [**`conduit.ai/config.json`**](https://conduit.ai/config.json).

La configurazione TUI usa [**`conduit.ai/tui.json`**](https://conduit.ai/tui.json).
...
Usa `CONDUIT_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `conduit.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `conduit serve` e `conduit web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server Conduit.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `conduit.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in Conduit tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, Conduit prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/conduit/agents/` o `.conduit/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, Conduit fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`conduit run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/conduit/commands/` o `.conduit/commands/`. [Scopri di piu](/docs/commands).
...
Conduit scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, conduit **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono Conduit con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.conduit/plugins/` o `~/.config/conduit/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare Conduit a usare solo alcuni provider invece di disabilitarli uno per uno.
