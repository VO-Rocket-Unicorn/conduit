---
title: Supporto ACP
description: Usa Conduit in qualunque editor compatibile con ACP.
---

Conduit supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare Conduit via ACP, configura l'editor in modo che esegua il comando `conduit acp`.

Il comando avvia Conduit come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'Conduit' nel selettore agenti di AI Chat.
...
Per usare Conduit come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare Conduit come agente ACP per la chat.

Se devi passare variabili d'ambiente (come `CONDUIT_API_KEY`), vedi [Configuring Adapters: Environment Variables](https://codecompanion.olimorris.dev/getting-started#setting-an-api-key) nella documentazione di CodeCompanion.nvim.

## Supporto

Conduit funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione Conduit
