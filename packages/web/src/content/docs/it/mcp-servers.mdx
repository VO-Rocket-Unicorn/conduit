...
Puoi aggiungere strumenti esterni a Conduit usando il _Model Context Protocol_, o MCP. Conduit supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Conduit Config](https://conduit.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/conduit`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Conduit gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Conduit:
...
Se il server richiede autenticazione, Conduit ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `conduit mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Conduit memorizzera' i token in modo sicuro in `~/.local/share/conduit/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Conduit insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Conduit come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Conduit al tuo account Sentry.
