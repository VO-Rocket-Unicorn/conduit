---
title: LSP Servers
description: Conduit integrates with your LSP servers.
---

Conduit can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

Conduit comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `CONDUIT_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and conduit opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your conduit config.
