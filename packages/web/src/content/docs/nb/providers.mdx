---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Conduit.
---

import config from "../../../../config.mjs"
export const console = config.console

Conduit bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Conduit-konfigurasjonen.
...
i `~/.local/share/conduit/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Conduit
konfig.
...
Conduit Zen er en liste over modeller levert av Conduit-teamet som har vært
testet og verifisert for å fungere godt med Conduit. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Conduit Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Conduit Zen` og gå til [conduit.ai/auth](https://conduit.ai/zen).
...
Det fungerer som alle andre leverandører i Conduit og er helt valgfritt å bruke.

---

## Conduit Go

Conduit Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Conduit-teamet som har vært
testet og verifisert for å fungere godt med Conduit.

1. Kjør kommandoen `/connect` i TUI, velg `Conduit Go`, og gå til [conduit.ai/auth](https://conduit.ai/zen).
...
Slik bruker du Amazon Bedrock med Conduit:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Conduit:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `conduit.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i conduit støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere conduit til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Conduit skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Conduit-konfigurasjonen.
...
for å lagre token i Conduit auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Conduit bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Conduit
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`conduit.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [conduit-gitlab-auth](https://www.npmjs.com/package/conduit-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `conduit.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Conduit:
...
Slik bruker du Google Vertex AI med Conduit:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Conduit.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Conduit, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [conduit-helicone-session](https://github.com/H2Shami/conduit-helicone-session) plugin for automatisk å logge hver Conduit-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Conduit-samtale oppført som en separat økt.
...
Du kan konfigurere Conduit for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Conduit for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Conduit for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Conduit. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/conduit) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Conduit:
...
3. Kopier API-nøkkelen for bruk i Conduit.
...
6. **Viktig**: Før du bruker skymodeller i Conduit, må du hente modellinformasjonen lokalt:
...
Conduit Zen er en liste over testede og verifiserte modeller levert av Conduit-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Conduit Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Conduit Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Conduit API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Conduit-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Conduit-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Conduit:
...
Du kan også tilpasse modeller gjennom Conduit-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Conduit. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `conduit.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Conduit forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `conduit auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Conduit-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Conduit-konfigurasjonen.
