...
Du kan legge til eksterne verktøy i Conduit ved å bruke _Model Context Protocol_ eller MCP. Conduit støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Conduit Config](https://conduit.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="conduit.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/conduit`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Conduit håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Conduit:
...
Hvis serveren krever autentisering, vil Conduit be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `conduit mcp auth <server-name>`.
...
```bash
conduit mcp auth my-oauth-server
```
...
```bash
conduit mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Conduit lagre tokens sikkert i `~/.local/share/conduit/mcp-auth.json`.
...
conduit mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Conduit, sammen med innebygde verktøy. Så du kan administrere dem gjennom Conduit-konfigurasjonen som alle andre verktøy.
...
```json title="conduit.json" {11, 14-18}
{
  "$schema": "https://conduit.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
conduit mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Conduit til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Conduit-samtale oppført som en separat økt.
