...
description: Skriv dine egne programtillegg for å utvide Conduit.
---

Plugins lar deg utvide Conduit ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Conduit.
...
- `.conduit/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/conduit/node_modules/`.
...
1. Global konfigurasjon (`~/.config/conduit/conduit.json`)
2. Prosjektkonfigurasjon (`conduit.json`)
3. Global plugin-katalog (`~/.config/conduit/plugins/`)
4. Prosjektpluginkatalog (`.conduit/plugins/`)
...
Conduit kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Conduit SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Conduit.
...
:::note
Hvis du bruker Conduit-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Conduit fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Conduit:
...
`tool`-hjelperen lager et tilpasset verktøy som Conduit kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Conduit sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://conduit.ai/docs/sdk) for detaljer.
