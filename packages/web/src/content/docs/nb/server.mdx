---
title: Tjener
description: Samhandle med Conduit-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `conduit serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Conduit-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `conduit.local` |
...
Sett `CONDUIT_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `conduit` som standard, eller sett `CONDUIT_SERVER_USERNAME` for å overstyre det. Dette gjelder både `conduit serve` og `conduit web`.
...
Når du kjører `conduit` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Conduit-serveren til å samhandle med Conduit programmatisk.
:::

Denne arkitekturen lar Conduit støtte flere klienter og lar deg samhandle med Conduit programmatisk.

Du kan kjøre `conduit serve` for å starte en frittstående server. Hvis du har
Conduit TUI kjører, vil `conduit serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Conduit [IDE](/docs/ide) plugins.
...
conduit-serveren viser følgende APIer.
