---
title: Policies
description: Control which configured resources Conduit may use.
---

Policies control whether Conduit may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `conduit.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether Conduit may use a resource such as an LLM provider.
...
Conduit currently supports one policy action:
