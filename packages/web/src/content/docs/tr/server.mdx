---
title: Sunucu
description: conduit sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`conduit serve` komutu, conduit istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `conduit.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `CONDUIT_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `conduit` değeridir; değiştirmek isterseniz `CONDUIT_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `conduit serve` hem de `conduit web` için geçerlidir.
...
`conduit` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
conduit ile programatik etkileşim için conduit sunucusunu kullanın.
:::

Bu mimari, conduit'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`conduit serve` ile bağımsız bir sunucu başlatabilirsiniz. conduit TUI açıksa, `conduit serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum conduit [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
conduit sunucusu aşağıdaki API'leri sunar.
