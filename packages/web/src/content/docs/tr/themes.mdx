...
conduit ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak conduit kendi `conduit` temasını kullanır.
...
conduit birden fazla yerleşik temayla gelir.
...
- conduit'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
conduit, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/conduit/themes/*.json` veya `$XDG_CONFIG_HOME/conduit/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.conduit/themes/*.json`
4. **Geçerli çalışma dizini** - `./.conduit/themes/*.json`
