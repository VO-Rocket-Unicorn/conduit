...
`.conduit/commands/test.md` oluştur:
...
conduit işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

conduit [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/conduit/commands/`
- Proje başına: `.conduit/commands/`
...
conduit, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
