...
Conduit Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Conduit'daki diğer sağlayıcılar gibi çalışır. Conduit Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Conduit'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Conduit Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Conduit Go, Conduit'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Conduit Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Conduit Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Conduit Go'ya abone olabilir.
:::
...
Conduit Go aşağıdaki limitleri içerir:
...
Conduit yapılandırmanızdaki [model id](/docs/config/#models) formatı `conduit-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `conduit-go/kimi-k2.6` kullanmalısınız.
...
Conduit Go'yu şu amaçlarla oluşturduk:
...
4. Conduit ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
