...
_Model Bağlam Protokolü_ veya MCP kullanarak conduit'a harici araçlar ekleyebilirsiniz. conduit hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [conduit Config](https://conduit.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/conduit` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
conduit, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde conduit şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa conduit, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `conduit mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından conduit, belirteçleri `~/.local/share/conduit/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra conduit'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi conduit yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve conduit'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
