...
description: Conduit, LSP sunucularınızla bütünleşir.
---

Conduit, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

Conduit, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`CONDUIT_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve conduit bir dosyayı açtığında:
...
LSP sunucularını, conduit yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
