...
Ajan becerileri, conduit'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
conduit su konumlari tarar:

- Proje konfigurasyonu: `.conduit/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/conduit/skills/<name>/SKILL.md`
...
Proje yerel yollarinda conduit, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.conduit/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/conduit/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.conduit/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
conduit, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`conduit.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`conduit.json` icinde):
