---
title: Sağlayıcılar
description: conduit'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

conduit, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı conduit yapılandırmanızda yapılandırın.
...
`~/.local/share/conduit/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları conduit'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Conduit Zen, conduit ekibi tarafından sağlanan modellerin bir listesidir.
conduit ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Conduit Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, conduit'u seçin ve [conduit.ai/auth](https://conduit.ai/auth)'ye gidin.
...
conduit'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Conduit Go

Conduit Go, conduit ile iyi çalıştığı test edilmiş ve doğrulanmış, conduit ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Conduit Go`'yu seçin ve [conduit.ai/auth](https://conduit.ai/zen) adresine gidin.
...
Amazon Bedrock'u conduit ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   conduit'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `conduit.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi conduit'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
conduit'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   conduit'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Conduit yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
conduit, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. conduit'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`conduit.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [conduit-gitlab-auth](https://www.npmjs.com/package/conduit-gitlab-auth) ana sayfasında.

##### Yapılandırma

`conduit.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi conduit'la kullanmak için:
...
Google Vertex AI'yi conduit ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları conduit çalıştırılırken ayarlayın.
...
Helicone'un conduit aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her conduit görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [conduit-helicone-session](https://github.com/H2Shami/conduit-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her conduit görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
conduit'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla conduit'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla conduit'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini conduit için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/conduit)'a bakın.
:::
...
Ollama Cloud'u conduit ile kullanmak için:
...
3. conduit'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: conduit'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Conduit Zen, conduit ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Conduit Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Conduit Zen**'i arayın.

   ```txt
   /connect
   ```

3. conduit API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   conduit yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca conduit yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Conduit ile kullanmak için:
...
Modelleri conduit yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı conduit ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `conduit.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları conduit'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `conduit auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için conduit yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, conduit yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
