---
title: Eklentiler
description: Conduit'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Conduit'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Conduit'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.conduit/plugins/` - Proje düzeyinde eklentiler
- `~/.config/conduit/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/conduit/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/conduit/conduit.json`)
2. Project config (`conduit.json`)
3. Global eklenti dizini (`~/.config/conduit/plugins/`)
4. Proje eklenti dizini (`.conduit/plugins/`)
...
Conduit bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Conduit SDK istemcisi.
...
Conduit'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Conduit masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Conduit'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Conduit'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Conduit'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Conduit'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://conduit.ai/docs/sdk) bakın.
