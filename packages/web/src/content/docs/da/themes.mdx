...
Med Conduit kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Conduit vårt eget `conduit`-tema.
...
Conduit kommer med flere innebygde temaer.
...
- Vil at Conduit skal matche terminalens utseende
...
Conduit støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/conduit/themes/*.json` eller `$XDG_CONFIG_HOME/conduit/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.conduit/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.conduit/themes/*.json`
