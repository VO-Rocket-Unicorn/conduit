...
Agentferdigheter lar Conduit oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Conduit søker etter disse stedene:

- Prosjektkonfigurasjon: `.conduit/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/conduit/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Conduit opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.conduit/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/conduit/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.conduit/skills/git-release/SKILL.md` slik:
...
Conduit viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `conduit.json`:
...
**For innebygde agenter** (i `conduit.json`):
