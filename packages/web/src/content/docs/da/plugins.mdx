---
title: Plugins
description: Skriv dine egne plugins for at udvide Conduit.
---

Plugins giver dig mulighed for at udvide Conduit ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Conduits standardadfærd.
...
- `.conduit/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/conduit/node_modules/`.
...
1. Global konfiguration (`~/.config/conduit/conduit.json`)
2. Projektkonfiguration (`conduit.json`)
3. Global plugin-mappe (`~/.config/conduit/plugins/`)
4. Projekt plugin bibliotek (`.conduit/plugins/`)
...
Conduit kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En conduit SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide conduit.
...
:::note
Hvis du bruger Conduit desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå conduit i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til conduit:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som conduit kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for conduit sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://conduit.ai/docs/sdk) for detaljer.
