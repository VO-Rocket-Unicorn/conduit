---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Conduit.
---

import config from "../../../../config.mjs"
export const console = config.console

Conduit bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Conduit-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/conduit/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Conduit-konfigurationen.
...
Conduit Zen er en liste over modeller leveret af Conduit-teamet, som er blevet testet og verificeret til at fungere godt med Conduit. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Conduit Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg conduit og gå til [conduit.ai/auth](https://conduit.ai/auth).
...
Det fungerer som alle andre udbydere i Conduit og er helt valgfrit at bruge.

---

## Conduit Go

Conduit Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Conduit-teamet, som er testet og verificeret til at fungere godt med Conduit.

1. Kør kommandoen `/connect` i TUI, vælg `Conduit Go`, og gå til [conduit.ai/auth](https://conduit.ai/zen).
...
Sådan bruger du Amazon Bedrock med Conduit:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører conduit:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `conduit.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Conduit understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere conduit til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at conduit skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem conduit-konfigurationen.
...
gemme tokenet i conduit auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Conduit bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Conduit
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`conduit.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [conduit-gitlab-auth](https://www.npmjs.com/package/conduit-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `conduit.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Conduit:
...
Sådan bruger du Google Vertex AI med Conduit:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem conduit, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [conduit-helicone-session](https://github.com/H2Shami/conduit-helicone-session) plugin for automatisk at logge hver Conduit-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Conduit-samtale opført som en separat session.
...
Du kan konfigurere conduit til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere conduit til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere conduit til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Conduit. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/conduit) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Conduit:
...
3. Kopier API-nøglen til brug i Conduit.
...
6. **Vigtigt**: Før du bruger skymodeller i Conduit, skal du hente modelinformationen lokalt:
...
Conduit Zen er en liste over testede og verificerede modeller leveret af Conduit-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Conduit Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Conduit Zen**.

   ```txt
   /connect
   ```

3. Indtast Conduit API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem conduit-konfigurationen.
...
5. Du kan også tilpasse dem gennem conduit-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Conduit:
...
Du kan også tilpasse modeller gennem conduit-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med conduit. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `conduit.json`-filen i projektkataloget:
...
`limit`-felterne lader Conduit forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `conduit auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek conduit-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i conduit-konfigurationen.
