---
title: Server
description: Interager med conduit-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `conduit serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en conduit-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `conduit.local` |
...
Indstil `CONDUIT_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `conduit`, eller indstil `CONDUIT_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `conduit serve` og `conduit web`.
...
Når du kører `conduit` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug conduit-serveren til at interagere med conduit programmatisk.
:::

Denne arkitektur lader conduit understøtte flere klienter og lader dig interagere med conduit programmatisk.

Du kan køre `conduit serve` for at starte en selvstændig server. Hvis du har
conduit TUI kørende, vil `conduit serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Conduit [IDE](/docs/ide) plugins.
...
Conduit-serveren viser følgende API'er.
