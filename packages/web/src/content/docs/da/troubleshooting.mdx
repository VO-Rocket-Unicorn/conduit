...
For at fejlfinde problemer med Conduit, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\conduit\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `conduit --log-level DEBUG`.

---

## Lagring

conduit gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/conduit/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\conduit`
...
Conduit Desktop kører en lokal Conduit-server (`conduit-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Conduit`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/conduit/conduit.jsonc` (eller `~/.config/conduit/conduit.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/conduit/conduit.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\conduit\conduit.jsonc`
...
Conduit kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/conduit/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\conduit\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Conduit kan genopbygge den.

1. Afslut Conduit Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/conduit`
- **Linux**: slet `~/.cache/conduit` (eller kør `rm -rf ~/.cache/conduit`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\conduit`

3. Start Conduit Desktop på ny.

---

### Løs problemer med serverforbindelse

Conduit Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `conduit.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `CONDUIT_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `CONDUIT_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Conduit Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Conduit.

---

### Meddelelser vises ikke

Conduit Desktop viser kun systemvarsler når:

- varsler er aktiveret for Conduit i dine OS-indstillinger, og
...
1. Afslut Conduit Desktop.
2. Find og slet disse filer (de findes i Conduit Desktop-appens datamappe):

- `conduit.settings.dat` (skrivebordsstandardserver URL)
- `conduit.global.dat` og `conduit.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Conduit:
...
For at finde ud af hvilke modeller du har adgang til, kør `conduit models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\conduit`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. conduit installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\conduit`

2. Start conduit på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge conduit til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
conduit vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
