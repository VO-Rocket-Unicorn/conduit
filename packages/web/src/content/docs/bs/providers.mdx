---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Conduit.
---

import config from "../../../../config.mjs"
export const console = config.console

Conduit koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Conduit konfiguraciji.
...
u `~/.local/share/conduit/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Conduit
config.
...
Conduit Zen je lista modela koje je obezbedio Conduit tim koji su bili
testirano i potvrđeno da dobro radi sa Conduit. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Conduit Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite conduit i idite na [conduit.ai/auth](https://conduit.ai/auth).
...
Radi kao i svaki drugi provajder u Conduit i potpuno je opcionalan za korištenje.

---

## Conduit Go

Conduit Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Conduit tim i koji su testirani i verificirani da dobro rade s Conduit-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Conduit Go` i idite na [conduit.ai/auth](https://conduit.ai/zen).
...
Da biste koristili Amazon Bedrock s Conduit:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete conduit:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `conduit.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Conduit.
:::
...
Možete konfigurirati conduit za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi conduit ispravno radio.
   :::
...
Također možete dodati modele kroz svoju conduit konfiguraciju.
...
da pohrani token u conduit auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Conduit koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Conduit
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`conduit.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [conduit-gitlab-auth](https://www.npmjs.com/package/conduit-gitlab-auth).

##### Konfiguracija

Prilagodite putem `conduit.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s conduit:
...
Za korištenje Google Vertex AI s Conduit:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće conduit.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem conduita, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [conduit-helicone-session](https://github.com/H2Shami/conduit-helicone-session) da automatski prijavite svaki Conduit razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Conduit razgovor naveden kao zasebna sesija.
...
Možete konfigurirati conduit za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati conduit za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati conduit za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Conduit. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/conduit) za detalje.
:::
...
Da biste koristili Ollama Cloud s Conduit:
...
3. Kopirajte API ključ za korištenje u Conduit.
...
6. **Važno**: Prije upotrebe modela oblaka u Conduit, morate lokalno povući informacije o modelu:
...
Conduit Zen je lista testiranih i verifikovanih modela koju je obezbedio Conduit tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Conduit Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Conduit Zen**.

```txt
   /connect
```

3. Unesite svoj Conduit API ključ.
...
Također možete dodati dodatne modele putem vaše conduit konfiguracije.
...
5. Također ih možete prilagoditi putem vaše conduit konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Conduitom:
...
Također možete prilagoditi modele kroz svoju conduit konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s conduit-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `conduit.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Conduit da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `conduit auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Conduit konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj conduit konfiguraciji.
