...
U Conduit mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Conduit koristi nasu `conduit` temu.
...
Conduit dolazi sa vise ugradenih tema.
...
- Zele da Conduit odgovara izgledu njihovog terminala
...
Conduit podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/conduit/themes/*.json` ili `$XDG_CONFIG_HOME/conduit/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.conduit/themes/*.json`
4. **Trenutni radni direktorij** - `./.conduit/themes/*.json`
