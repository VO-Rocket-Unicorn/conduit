...
Da biste otklonili probleme s Conduit, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\conduit\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `conduit --log-level DEBUG`.

---

## Pohrana

Conduit pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/conduit/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\conduit`
...
Conduit Desktop pokreće lokalni Conduit server (`conduit-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Conduit` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/conduit/conduit.jsonc` (ili `~/.config/conduit/conduit.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/conduit/conduit.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\conduit\conduit.jsonc`
...
Conduit također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/conduit/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\conduit\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Conduit mogao ponovo izgraditi.

1. Potpuno zatvorite Conduit Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/conduit`
- **Linux**: obrišite `~/.cache/conduit` (ili pokrenite `rm -rf ~/.cache/conduit`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\conduit`

3. Ponovo pokrenite Conduit Desktop.

---

### Rješavanje problema sa vezom na serveru

Conduit Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `conduit.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `CONDUIT_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `CONDUIT_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Conduit Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Conduit karakteristikama.

---

### Obavještenja se ne prikazuju

Conduit Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Conduit u postavkama vašeg OS-a, i
...
1. Zatvorite Conduit Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Conduit Desktop aplikacije):

- `conduit.settings.dat` (URL zadanog servera za desktop)
- `conduit.global.dat` i `conduit.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `conduit models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\conduit`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Conduit dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\conduit`

2. Ponovo pokrenite Conduit da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Conduit da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Conduit će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
