...
Agent skills omogucavaju Conduit da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Conduit pretrazuje ove lokacije:

- Konfiguracija projekta: `.conduit/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/conduit/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Conduit ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.conduit/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/conduit/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.conduit/skills/git-release/SKILL.md` ovako:
...
Conduit navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `conduit.json`:
...
**Za ugradene agente** (u `conduit.json`):
