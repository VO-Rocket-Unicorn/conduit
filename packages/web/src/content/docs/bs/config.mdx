---
title: Konfiguracija
description: Korištenje Conduit JSON konfiguracije.
---

Možete konfigurirati Conduit koristeći JSON konfiguracijski fajl.

---

## Format

Conduit podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/conduit`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/conduit/conduit.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`CONDUIT_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`conduit.json` u projektu) - postavke specifične za projekat
5. **`.conduit` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`CONDUIT_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.conduit` i `~/.config/conduit` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/conduit` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Conduit konfiguraciju u `~/.config/conduit/conduit.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/conduit/tui.json`.
...
Dodajte `conduit.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Conduit pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `CONDUIT_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `CONDUIT_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.conduit` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.conduit`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`conduit.ai/config.json`**](https://conduit.ai/config.json).

TUI konfiguracija koristi [**`conduit.ai/tui.json`**](https://conduit.ai/tui.json).
...
Koristite `CONDUIT_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `conduit.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `conduit serve` i `conduit web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Conduit server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `conduit.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Conduit konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Conduit pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/conduit/agents/` ili `.conduit/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Conduit će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`conduit run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/conduit/commands/` ili `.conduit/commands/`. [Saznajte više ovdje](/docs/commands).
...
Conduit će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Conduit **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Conduit sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.conduit/plugins/` ili `~/.config/conduit/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Conduit da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
