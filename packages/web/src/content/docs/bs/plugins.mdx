---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Conduit.
---

Dodaci vam omogućavaju da proširite Conduit spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Conduit.
...
- `.conduit/plugins/` - Dodaci na nivou projekta
- `~/.config/conduit/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/conduit/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/conduit/conduit.json`)
2. Konfiguracija projekta (`conduit.json`)
3. Globalni direktorij dodataka (`~/.config/conduit/plugins/`)
4. Direktorij dodataka projekta (`.conduit/plugins/`)
...
Conduit pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Conduit SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Conduit.
...
Spriječite conduit da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u conduit:
...
Pomoćnik `tool` kreira prilagođeni alat koji conduit može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za conduit zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://conduit.ai/docs/sdk) za detalje.
