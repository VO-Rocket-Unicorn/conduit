...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Conduit dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Conduit.
...
:::note
Ovaj alat je dostupan samo kada je `CONDUIT_EXPERIMENTAL_LSP_TOOL=true` (ili `CONDUIT_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Conduit provajdera ili kada je varijabla `CONDUIT_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Conduit:
