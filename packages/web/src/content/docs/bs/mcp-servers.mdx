...
Možete dodati vanjske alate u Conduit koristeći _Model Context Protocol_, ili MCP. Conduit podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Conduit Config](https://conduit.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/conduit`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Conduit automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Conduit će:
...
Ako server zahtijeva autentifikaciju, Conduit će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `conduit mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Conduit sigurno čuva tokene u `~/.local/share/conduit/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Conduit, zajedno s ugrađenim alatima. Možete njima upravljati kroz Conduit konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Conduit sa vašim Sentry nalogom.
