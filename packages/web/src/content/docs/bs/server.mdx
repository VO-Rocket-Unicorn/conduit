---
title: Server
description: Komunicirajte s conduit serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `conduit serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji conduit klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `conduit.local` |
...
Postavite `CONDUIT_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `conduit`, ili postavite `CONDUIT_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `conduit serve` i za `conduit web`.
...
Kada pokrenete `conduit`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite conduit server za programsku interakciju sa conduit-om.
:::

Ova arhitektura omogucava conduit podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `conduit serve` da startate standalone server. Ako je conduit TUI vec pokrenut, `conduit serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Conduit [IDE](/docs/ide) pluginovi.
...
conduit server izlaže sljedece API-je.
