...
Создайте `.conduit/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию conduit или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем conduit [config](/docs/config):
...
- Глобальный: `~/.config/conduit/commands/`
- Для каждого проекта: `.conduit/commands/`
...
conduit включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
