...
# パッケージマネージャー
npm i -g conduit-ai@latest        # bun/pnpm/yarn でもOK
scoop install conduit             # Windows
choco install conduit             # Windows
brew install anomalyco/tap/conduit # macOS と Linux（推奨。常に最新）
brew install conduit              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S conduit            # Arch Linux (Stable)
paru -S conduit-bin               # Arch Linux (Latest from AUR)
mise use -g conduit               # どのOSでも
nix run nixpkgs#conduit           # または github:anomalyco/conduit で最新 dev ブランチ
```
...
Conduit はデスクトップアプリとしても利用できます。[releases page](https://github.com/anomalyco/conduit/releases) から直接ダウンロードするか、[conduit.ai/download](https://conduit.ai/download) を利用してください。
...
1. `$CONDUIT_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.conduit/bin` - デフォルトのフォールバック
...
Conduit には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://conduit.ai/docs/agents) の詳細はこちら。

### ドキュメント

Conduit の設定については [**ドキュメント**](https://conduit.ai/docs) を参照してください。

### コントリビュート

Conduit に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### Conduit の上に構築する

Conduit に関連するプロジェクトで、名前に "conduit"（例: "conduit-dashboard" や "conduit-mobile"）を含める場合は、そのプロジェクトが Conduit チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。

---

**コミュニティに参加** [Discord](https://discord.gg/conduit) | [X.com](https://x.com/conduit)
